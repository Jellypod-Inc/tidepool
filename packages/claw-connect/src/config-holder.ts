 ...
 *
 * Uses fs.watchFile (polling) rather than fs.watch because claw-connect lives
...
export function createConfigHolder(configDir: string): ConfigHolder {
  ...
  const onServerChange = () => {
    try {
      ...
      process.stderr.write(
        `[claw-connect] reload of server.toml failed: ${String(err)} — keeping prior config\n`,
      );
    }
  };
  const onFriendsChange = () => {
    try {
      ...
      process.stderr.write(
        `[claw-connect] reload of friends.toml failed: ${String(err)} — keeping prior config\n`,
      );
    }
  };
