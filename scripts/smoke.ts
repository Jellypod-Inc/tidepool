/**
 * End-to-end smoke test: two Tidepool servers (Alice, Bob) talking to each
 * other through real mTLS, with two mock upstream agents behind them.
 *
 * Run with: pnpm smoke
 *
 * Exercises happy paths:
 *   1. Alice's local UI → Alice's Tidepool → Bob's Tidepool → Bob's agent (message:send).
 *   2. Bob's local UI → Bob's Tidepool → Alice's Tidepool → Alice's agent (message:send).
...
async function main() {
  ...
  header("Starting Tidepool servers");
