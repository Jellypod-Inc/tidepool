...
Two project directories, each with its own Claude Code session. One is "alice", one is "bob". When alice sends an A2A message, bob's Claude sees it and can reply — and vice versa. A single `tidepool` daemon runs in the background and routes between them; you don't interact with it directly.
...
Open a second terminal, `cd` into a different project, run the same command. You get a different animal name (or supply one) and a second Claude session that shares the single background `tidepool` daemon. Messages between the two sessions hop through `127.0.0.1` with no TLS.
...
In the other terminal you'll see a `<channel source="tidepool" peer="alice" context_id="…" task_id="…" message_id="…">` block appear. Claude responds by calling the `send` tool with `peers=["alice"]` and `thread=<context_id>` (the same `context_id` from the inbound event). The reply routes back to the first session, where it arrives as its own `<channel source="tidepool" …>` event sharing the same `context_id`.
...
**`tidepool claude-code:start` prints "claude is not on your PATH".** The setup still succeeded — agent is registered, daemon is running, `.mcp.json` is in place. Copy the `cd <dir> && claude …` command it printed and run it in a fresh terminal. Installing Claude Code so `which claude` finds it removes this branch.

**`tidepool claude-code:start` exits with "Tidepool did not become ready within 3000ms".** The daemon spawned but its local port never responded. Check the log file at `~/.config/tidepool/logs/serve-<date>.log` for a crash. The most common causes are port 9900 or 9901 already in use — `lsof -i :9901` tells you who. Either stop that process or edit `~/.config/tidepool/server.toml` to change `port`/`localPort`, then rerun. `claude-code:start --debug` bypasses the daemon entirely so you can see serve's output live.
...
**Want to start fresh.** `tidepool stop && rm -rf ~/.config/tidepool`. Also delete the `.mcp.json` in each project. Then start over.
...
- The adapter's HTTP listener binds to `127.0.0.1` only. Trust for cross-machine traffic is delegated to tidepool's mTLS.
